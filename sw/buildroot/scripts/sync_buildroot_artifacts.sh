...
copy_required "${IMAGES_DIR}/boot.bin" "${ARTIFACTS_DIR}/boot-spl.bin"
copy_required "${IMAGES_DIR}/u-boot.img" "${ARTIFACTS_DIR}/u-boot.img"
copy_required "${IMAGES_DIR}/tfg_zedboard_bd_wrapper.bit" "${ARTIFACTS_DIR}/tfg_zedboard_bd_wrapper.bit"

echo "Artefactos de Buildroot sincronizados en: ${ARTIFACTS_DIR}"
echo "Nota: boot-spl.bin es el primer cargador activo en el flujo U-Boot SPL."
