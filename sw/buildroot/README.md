...
La primera build completa ya se ha ejecutado correctamente y ha generado `host/`, `target/` e `images/` dentro de `sw/buildroot/output/zedboard/`. Esta ejecución valida que el flujo base de Buildroot funciona con la configuración actual del proyecto.

## Artefactos congelados

Los artefactos relevantes de esta build se han copiado a `artifacts/buildroot/` para conservar una salida trazable de `T7`:

- `u-boot.elf`: ELF de U-Boot generado por Buildroot.
- `uImage`: kernel Linux en formato U-Boot legacy image.
- `system.dtb`: DTB usado por el arranque, actualmente equivalente a `zynq-zed.dtb`.
- `zynq-zed.dtb`: device tree base de ZedBoard generado por Buildroot.
